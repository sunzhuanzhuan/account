...
const UpdateOwnerPage = props => {
  ...
  const getPhoneList = () => {
    setListLoading(true);
    props.actions
      .phoneUpdateHistory({ mcnId: linkUserId })
      .then(({ data }) => {
        setPhoneData(data);
      })
      .finally(() => {
  ...
  const menu = (
    <Menu>
      ...
      {showButton.policy && (
        <Menu.Item key="2">
          <Link target="_blank" to={`/account/policy/list/${linkUserId}`}>
            采购政策
          </Link>
        </Menu.Item>
      )}
      {showButton.channelDiscount > 0 && (
        <Menu.Item key="4">
          <Link target="_blank" to={`/account/discount?userId=${linkUserId}`}>
            修改渠道折扣
          </Link>
        </Menu.Item>
      )}
      {showButton.channelDiscount < 0 && (
        <Menu.Item key="5">
          <Link target="_blank" to={`/account/discount?userId=${linkUserId}&name=${linkUserName}`}>
            添加渠道折扣
          </Link>
        </Menu.Item>
      )}
          ...
          <div>
            {showButton.grossPayment > 0 && (
              <Button
                type="primary"
                target="_blank"
                href={`${linkHost}/payment/manager/paymentlist/serviceProvider/1/userId/${linkUserId}`}
              >
                修改自营支付方式
              </Button>
            )}
            ...
            {showButton.netPayment > 0 && (
              <Button
                type="primary"
                target="_blank"
                href={`${linkHost}/payment/manager/paymentlist/serviceProvider/2/userId/${linkUserId}`}
              >
                修改直供支付方式
              </Button>
            )}
...
export default connect(mapStateToProps, mapDispatchToProps)(UpdateOwnerPage);
