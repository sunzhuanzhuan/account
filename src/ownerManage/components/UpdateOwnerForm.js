...
const UpdateOwnerForm = props => {
  ...
  let initUserTypeId = keys.includes(props.userType) ? props.userType : 0;
  const InfoPay = (
    <QuestionTip
      title="支付信息"
      content={
  ...
  const save = values => {
    setLoading(true);
    props.actions
      .ownerUpdate(handleValues(values))
      .then(() => {
        setLoading(false);
      ...
      if (!err) {
        if (diffMcn) {
          Modal.confirm({
            title: '提示',
            content:
              '此主账号所属pack分属多个媒介经理，pack将转移给此次转移主账号目标媒介经理，确认转移？',
            onOk: () =>
  ...
  return (
    <Form
      {...formItemLayout}
      onSubmit={props.disabled ? () => {} : handleSubmit}
      className=""
      colon={false}
    >
      <Form.Item label="资源媒介">
        {getFieldDecorator('ownerAdminId', {
          ...
          rules: [{ required: true, message: '请选择资源媒介' }]
        })(
          <Select placeholder="请选择" onChange={handleDiffMcn} disabled={props.disabled}>
            {props.ownerAdminId > 0 && (
              <Option key={props.ownerAdminId} value={props.ownerAdminId}>
                {props.ownerAdminName}
              </Option>
            )}
        ...
        <a className="form-suffix-action-70" onClick={() => props.setModal('media')}>
          修改历史
        </a>
      </Form.Item>
      ...
      <Form.Item label="主账号类型">
        {getFieldDecorator('userType', {
          ...
          rules: [{ required: true, message: '主账号类型不能为空' }]
        })(
      ...
      <Form.Item label={<span>支付信息{InfoPay}</span>}>
        {getFieldDecorator('taxInPrice', {
          ...
          rules: [{ required: true, message: '请选择支付信息！' }]
        })(
          <RadioGroup disabled={props.disabled || props.paymentInfoIsComplete === 1}>
            <a
              className="form-suffix-action-70"
              style={{ marginRight: -50 }}
              onClick={() => props.setModal('payment')}
            >
              修改历史
            </a>
            <Radio value={1}>
              报价<span style={{ color: '#f00' }}>含税</span>，后期须提供增值税专用发票后方可提现
            </Radio>
            <br />
            <Radio value={2}>
              报价<span style={{ color: '#f00' }}>不含税</span>
              ，提现须授权微播易相关通道平台代扣代缴综合税费
            </Radio>
          </RadioGroup>
        )}
      </Form.Item>
      {getFieldValue('taxInPrice') === 1 && (
        <Form.Item label="回票类型">
          {getFieldDecorator('invoiceType', {
            initialValue: props.invoiceType || undefined,
            rules: [{ required: true, message: '请选择回票类型！' }]
          })(
            <RadioGroup disabled={props.disabled || props.paymentInfoIsComplete === 1}>
      ...
      {getFieldValue('invoiceType') === 1 && (
        <Form.Item label="发票税率">
      ...
      <Form.Item label="手机号码">
        ...
        <a className="form-suffix-action-70" onClick={() => props.setModal('cellPhone')}>
          修改历史
        </a>
      </Form.Item>
      {diffPhone && (
      ...
      <ContactTypesLeastOne
        form={props.form}
        qq={props.qq}
        weixinId={props.weixinId}
        email={props.email}
        disabled={props.disabled}
      />
      ...
      <Form.Item label="其他联系人" wrapperCol={{ xs: { span: 24 }, sm: { span: 20 } }}>
        <ContactExtend
          form={props.form}
          data={props.mcnContactInfoList}
          disabled={props.disabled}
        />
      </Form.Item>
      {props.disabled ? null : (
        <div style={{ textAlign: 'center', padding: 20 }}>
          <Button type="primary" htmlType="submit" loading={loading} style={{ width: '20%' }}>
            提交
          </Button>
        </div>
      )}
