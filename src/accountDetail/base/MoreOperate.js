...
class MoreOperate extends Component {
  ...
  componentDidMount = () => {
    this.showMore();
    events.on('message', this.showMore)
  }
  componentWillUnmount() {
    events.removeListener('message', this.showMore)
  }
  showMore = () => {
  ...
  render() {
    ...
    return (
      <div ref={e => this.parent = e} style={{ position: 'relative', height: isTop ? "auto" : lineHight, overflow: "hidden" }}>
        <div ref={e => this.children = e} style={{ width: isShowMoreText ? "94%" : "", float: "left", height: isTop ? "auto" : lineHight }} >
          {this.props.children}
        </div>
        {isShowMoreText ? <div style={{ width: 40, position: "absolute", right: 0 }}>
