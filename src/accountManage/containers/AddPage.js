...
class AddPage extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (values.extend) {
          values.extend['followerCountScreenshotUrl'] = uploadUrl(
            values.extend['followerCountScreenshotUrl']
          );
  ...
  componentWillMount() {
    ...
    Promise.all([
      ...
      getSkuTypeList({ platformId: pid, productLineId: 2 })
    ]).then(() => {
  ...
  addQuote = () => {
    this.props.history.push(
      `/account/manage/update/${this.state.addQuoteData.platformId}?account_id=${
        this.state.addQuoteData.accountId
      }&addQuote=addQuote`
    );
  };
  render() {
    ...
    return (
      <Form onSubmit={this.handleSubmit}>
        {loading ? (
          ...
          <AddPageCommonContainer params={params}>
            <Route
              path={'/account/manage/add/:pid'}
              render={() => <RouteViewChild params={params} form={form} />}
            />
        ...
        >
          ...
          <Button className="addPage-tips-modal-button" type="primary" onClick={this.addQuote}>
            立即添加报价
          </Button>
        </Modal>
      </Form>
