...
class UpdatePage extends Component {
  ...
  componentDidMount() {
    ...
    let accountId = (this.accountId = qs.parse(this.props.location.search.substring(1))[
      'account_id'
    ]);
  ...
  render() {
    ...
    if (platformId && this.pid) {
      if (platformId != this.pid) {
        window.location.replace(
          '/account/manage/update/' + platformId + '?account_id=' + this.accountId
        );
        // window.reload()
      }
    }
    return loading ? (
      ...
      <UpdatePageCommonContainer params={params}>
        <Route
          path={'/account/manage/update/:pid'}
          render={() => (
            <RouteViewChild
              platformId={this.pid}
              params={params}
              addQuote={this.addQuote ? this.addQuote : undefined}
            />
          )}
        />
      </UpdatePageCommonContainer>
    ) : (
      <h2>{errorMsg}</h2>
    );
  }
