...
export class AccountInfos extends Component {
  render() {
    ...
    let href = isOwner
      ? `${babysitterHost}/user/index/type/huanma/account_id/${accountId}/weibo_type/${platformId}`
      : `${babysitterHost}/user/chowner/account_id/${accountId}`;

    const rightC = (
      <div className="wrap-panel-right-content">
    ...
    return (
      <div className="account-info-container update-page">
        <div>
          <WrapPanel header="主账号信息" navId="mainAccountInfos" right={rightC}>
            <MainAccountInfos accountInfo={accountInfo} />
          </WrapPanel>
          {React.Children.map(this.props.children, child =>
            React.cloneElement(child, { refresh: this.props.refresh })
          )}
        </div>
        {this.props.sidebarData.length ? (
          <AffixNav
            scrollNode=".account-info-container"
            isUpdate={true}
            actions={actions}
            current={this.props.navCurrent}
            dataSource={this.props.sidebarData}
...
export class FetchInfoForm extends Component {
  render() {
    ...
    return (
      <WrapPanel navId="getAccountInfos" right={rightC} header={FetchHead}>
...
export class BaseInfoForm extends Component {
  ...
  handleValues = values => {
    ...
    const { accountId, platformId } = accountInfo;
    values['id'] = accountId;
  ...
  render() {
    ...
    const baseInfoLeft = (
      <div className="wrap-panel-left-content">
        <img
          style={{
            position: 'relative',
            top: '-3px'
...
export class AccountPriceForm extends Component {
  ...
  submit = e => {
    e.preventDefault();
    const {
      data: { priceInfo, accountInfo }
    } = this.props.params;
    const { skuList } = priceInfo;
    const { accountId, isFamous, platformId } = accountInfo;
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
  ...
  render() {
    ...
    const { isFamous } = accountInfo;
    const { skuList, modifiedAt } = priceInfo;
    const priceKeys = skuList
      ? skuList.map(({ skuTypeId, skuTypeName }) => ({
          key: skuTypeId,
          name: skuTypeName
        }))
      : [];
        ...
        <WrapPanel header="账号报价" right={rightC}>
          {_isFamous ? (
            <FamousPrice {...params} {...form} priceKeys={priceKeys}>
              {diff.referencePrice ? (
                <ReferencePrice {...params} {...form} />
              ) : (
                <i style={{ display: 'none' }} />
              )}
              {diff.priceInclude ? (
                <PriceInclude {...params} {...form} />
              ) : (
                <i style={{ display: 'none' }} />
              )}
...
export class AccountFeatureForm extends Component {
  webpackExtraFormData = values => {
    ...
    if (values.extend) {
      ...
      values.extend.birthDate = values.extend.birthDate
        ? values.extend.birthDate.format('YYYY-MM-DD')
        : '';
    } else {
      // 没有展开必须传递一个空的extend
      values.extend = {};
...
export class CooperateInfoForm extends Component {
  onSave = values => {
    ...
    /*values.cooperationCases = cooperationCases.filter(({isDeletedd}) => !isDeletedd).map((item, n) => {
			item.index = n
			return item
		})*/
    values.cooperationCases = cooperationCases; /*.filter(({isDeletedd}) => !isDeletedd)*/
    return values;
  };
  ...
  render() {
    const { form, params } = this.props;
    const {
      actions: { updateAccountCooperation }
    } = params;
    return (
      <WrapPanelForm
        header="合作信息"
        navId="cooperationInfos"
        onSave={this.onSave}
        action={updateAccountCooperation}
      >
...
export class AudiencePortraitForm extends Component {
  ...
  submit = e => {
    e.preventDefault();
    const {
      actions: { updateAccountAudiencePortrait }
    } = this.props.params;
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        let data = this.handleData(values);
        this.setState({ isLoading: true });
        updateAccountAudiencePortrait({ ...data })
          .then(() => {
  ...
  render() {
    ...
    const { latestPublishTime } = accountInfo;
    const rightC = (
      <div className="wrap-panel-right-content">
        <span className="gray-text">信息更新时间 : {latestPublishTime || '--'}</span>
        <Button loading={this.state.isLoading} size="small" type="primary" onClick={this.submit}>
          {'保存'}
        </Button>
      </div>
    );
