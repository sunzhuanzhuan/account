...
export default class UpdateChild extends Component {
  ...
  getSkuActions = () => {
    ...
    const { accountId, isFamous } = accountInfo;
  ...
  render() {
    ...
    const {
      ...
      actions: { updateAccountFans, getAccountOnShelfStatus }
    } = params;
    const { priceValidTo } = priceInfo;
    const {
      isFamous,
      accountId,
      isOnline,
      ...
      >
        <TabPane tab="帐号信息" key="1">
          <AccountInfos params={params}>
            {fetch ? (
              <FetchInfoForm params={params} diff={fetch} />
            ) : (
              <i style={{ display: 'none' }} />
            )}
            <BaseInfoForm params={params} diff={base} />
            <WrapPanelForm
              onSave={this.saveFans}
              navId="fansInfos"
              header="粉丝信息"
              action={updateAccountFans}
            >
              <Fans
                {...params}
        ...
        <TabPane
          ...
          tab={
            <span>
              报价信息
              {isFamous == 1 && hasRedDot(priceValidTo) ? (
                <Badge dot>
